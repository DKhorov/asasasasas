...
export default function AtomsClicker() {
  ...
  const [myId, setMyId] = useState(null); // ✅ добавили состояние ID
  ...
  useEffect(() => {
    // Загружаем свой профиль через /auth/me
    const fetchMe = async () => {
      try {
            ...
            <React.Fragment key={chat._id || i}>
              <ListItem onClick={() => selectChat(chat)} sx={{ "&:hover": { bgcolor: "#1e1e1e" }, bgcolor: currentChat?._id === chat._id ? "#1e1e1e" : "transparent" }}>
                <Badge overlap="circular" anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }}
                  ...
                  <Avatar alt={other?.fullName || 'Пользователь'} src={other?.avatarUrl || ""} sx={{ width: 50, height: 50 }} />
                </Badge>
                <Box sx={{ ml: 1, flex: 1 }}>
                  <ListItemText
  ...
  return (
    <>
      <Box sx={{ display: 'flex', height: '100vh', width: '100%', bgcolor: "#000" }}>
        {isMobile ? (

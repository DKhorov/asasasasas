...
const RegistrationPage = () => {
 ...
 useEffect(() => {
  ...
  // Когда компонент размонтируется — удаляем бейдж и сам скрипт
  return () => {
    // Удаляем скрипт
    document.body.removeChild(script);

    // Удаляем или скрываем бейдж, если он появился
  ...
  const nextStep = async () => {
    ...
    // Шаг с правилами и регистрацией
    if (STEPS[step] === 'rules') {
      if (!agree) {
    ...
    // Для остальных шагов просто идем дальше
    setShow(false);
    setLoading(true);
    setTimeout(() => {
      setLoading(false);
